...
describe("permission 页面测试", () => {
  ...
  it("测试新增权限功能", async () => {
    ...
    // 只写必填项
    // 输入权限名称
    const addPermissionName = "权限(测试)" + Math.random().toFixed(2);
    const addNameInput = await testPage.$('input[id="permission-form-name"]');
    await addNameInput?.type(addPermissionName);

    // 输入权限编码
    const addPermissionCode = "permission_code_" + Math.random().toString(36).substring(2, 10);
    const addCodeInput = await testPage.$('input[id="permission-form-code"]');
    await addCodeInput?.type(addPermissionCode);
    ...
    const addedSpanText = await testPage
      ...
      .wait();
    expect(addedSpanText).toContain(addPermissionName);
  });
  ...
  it("测试编辑权限功能", async () => {
    ...
    const editPermissionName = "权限(测试)" + Math.random().toFixed(2);
