...
describe("角色管理页面测试", () => {
  ...
  it("测试新增角色功能", async () => {
    ...
    // 只写必填项
    // 输入角色名称
    const roleName = "测试角色" + Math.floor(Math.random() * 10000);
    const addNameInput = await testPage.$('input[id="role-form-name"]');
    await addNameInput?.type(roleName);

    // 输入角色编码
    const roleCode = "ROLE_TEST_" + Math.floor(Math.random() * 10000);
    const addCodeInput = await testPage.$('input[id="role-form-code"]');
    await addCodeInput?.type(roleCode);
    ...
    const addedSpanText = await testPage
      ...
      .wait();
    expect(addedSpanText).toContain(roleName);
  });
  ...
  it("测试编辑角色功能", async () => {
    ...
    // 等待获取角色详情接口返回
    await new Promise(resolve => setTimeout(resolve, 500));
    const editRoleName = "测试角色" + Math.floor(Math.random() * 10000);
